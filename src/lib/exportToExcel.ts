...
export const exportToExcel = (reportData: ReportData, revenueData: RevenueData, reportId: string) => {
    ...
    ws_data[currentRow + 1] = [{ v: "المشروع", s: styles.label }, { v: "صف السيارات - جبل عمر", s: styles.value }];
    ...
    const revenueTableHeaders = ["الموقع", ...(isMonthlyReport ? [] : ["الكاشير"]), "السيارات", "الزوار", "النزلاء", "مبالغ المواقف", "مبالغ الفاليه", "المجموع"];
    ws_data[currentRow] = revenueTableHeaders.map(h => ({ v: h, s: styles.tableHeader }));
    currentRow++;

    hotels.forEach(hotel => {
        const revenue = revenueData.revenueByHotel[hotel] || { cars: 0, visitors: 0, guests: 0, parking: 0, valet: 0, total: 0, cashierName: '-' };
        ...
        const row = [
            ...
            { v: revenue.visitors || 0, s: styles.tableCell, t: 'n' },
            { v: revenue.guests || 0, s: styles.tableCell, t: 'n' },
    ...
    const totalVisitors = Object.values(revenueData.revenueByHotel).reduce((acc, { visitors }) => acc + (visitors || 0), 0);
    const totalGuests = Object.values(revenueData.revenueByHotel).reduce((acc, { guests }) => acc + (guests || 0), 0);
    ...
    const grandTotal = Object.values(revenueData.revenueByHotel).reduce((acc, { total }) => acc + (total || 0), 0);
    ...
    ws_data[currentRow] = [
        ...
        { v: totalVisitors, s: { ...styles.totalRow, ...styles.tableCell }, t: 'n' },
        { v: totalGuests, s: { ...styles.totalRow, ...styles.tableCell }, t: 'n' },
        { v: totalParking, s: { ...styles.totalRow, ...styles.tableCell }, t: 'n', z: '#,##0.00' },
        { v: totalValet, s: { ...styles.totalRow, ...styles.tableCell }, t: 'n', z: '#,##0.00' },
        { v: grandTotal, s: { ...styles.totalRow, ...styles.tableCell }, t: 'n', z: '#,##0.00' },
    ];
    currentRow+=2;
